    ...
    <div className="group relative card card-hover overflow-hidden">
      <div className="relative aspect-4/3 w-full overflow-hidden bg-linear-to-br from-slate-100 to-slate-50">
        ...
        <div className="w-full h-full flex items-center justify-center">
                  {/* circle icon */}
          <div className="w-20 h-20 rounded-full bg-linear-to-br from-purple-100 to-slate-100 flex items-center justify-center">
            <span className="text-2xl font-medium text-purple-400">{employee.firstName[0]} {employee.lastName[0]}</span>
          </div>
        </div>
      </div>
      ...
      {!employee.isDeleted && (
        <div className="absolute inset-0 bg-linear-to-t from-purple-700/20 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end justify-center pb-6 gap-3">
<button onClick={()=>onEdit(employee)} className="p-2.5 bg-white/90 backdrop-blur-sm text-slate-700 hover:text-purple-600 rounded-xl shadow-lg transition-all hover:scale-105">
