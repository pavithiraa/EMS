    ...
    <div className="animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-5  mb-8">
        ...
        <div key={index} className="card card-hover p-5 sm:p-6 relative overflow:hidden group flex items-center justify-between">
            <div>
                <div className="absolute left-0 top-0 bottom-0 w-1 rounded-r-full bg-slate-500/70 group-hover:bg-purple-500/70"/>
                <p className="text-sm font-medium text-slate-700">{card.title}</p>
                <p className="text-2xl font-bold text-slate-900">{card.value}</p>
            </div>
            <card.icon className="size-10 p-2.5 rounded-lg bg-slate-100 text-slate-600 group-hover:bg-purple-50 group-hover:text-purple-600 transition-colors duration-200"/>
        </div>
        ))}
      </div>
