    ...
    <>
      ...
      <div className="px-5 pt-6 pb-5 border-b border-white/6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-semibold text-[13px] text-whote tracking-wide">
                Employee MS
              </p>
      ...
      <div className="flex-1 px-3 space-y-0.5 overflow-y-auto">
           {navItems.map((item)=>{
              ...
              return(
                <Link key={item.name} to={item.href} className={`group flex items-center gap-3 px-3 py-2.5 rounded-md text-[13px] font-medium transition-all duration-150 relative ${isActive ? "bg-purple-500/12 text-purple-300":"text-slate-300 hover:text-white hover:bg-white/4"}`}>
            {isActive && <div className="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-5 rounded-r-full bg-purple-500"/>}
            <item.icon className={`w-[17px] h-[17px] shrink-0 ${isActive ? "text-purple-300":"text-slate-400 group-hover:text-slate-300"}`}/>
                <span className="flex-1">{item.name}</span>
                {isActive && <ChevronRightIcon className="w-3.5 h-3.5 text-purple-500/50"/>}
                </Link>
              )
           })}
      </div>
